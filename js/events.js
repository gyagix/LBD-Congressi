    ...
    */ 
	...
	return {
		...
		fields:[
				...
				{ name: 'Language', type: 'text', label: 'Lingua', showInTableList:false },
				{ name: 'Email', type: 'text', label: 'Email', showInTableList:false }
				/*
				,{ name: 'IdVenue', type: 'number', label: 'Id Venue', showInTableList:false }
				,{ name: 'Venue_Code', type: 'text', label: 'Venue Code', showInTableList:false }
				*/
		],
		...
		async init() {
			...
			this.editContainer = document.getElementById("eventEditContainer"); 
		...
		updateCodeVenue() {
			const venue = this.venueOptions.find(v => v.Id === this.form.IdVenue);
			this.form.CodeVenue = venue?.Code || '';
		},
		...
		async saveEvent() {
			try {
				...
				} else if (payload.IdVenue) {
				payload.Venue = payload.IdVenue;
				}

				if (payload.Venue != null && payload.Venue !== '') {
